...
9) Отправка сообщений через Dmail (отправка происходит самому себе, письма не приходят, взаимодействия только с контрактом) респект за код https://github.com/sybil-v-zakone/dmailer

10) L2Telegraph минт + бридж NFT

11) L2Telegraph отправка сообщений

12) Деплой контракта токена + его минт

13) Возможность подключения к прокси

14) Логирование через logger

15) Чекер газа перед началом работы

16) Возможность создать кастомные маршруты (действия которые будут выполняться последовательно, настраивается в settings.py)
---
<h2>⚙️ Настройка</h2>
