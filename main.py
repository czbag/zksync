...
def get_module():
    result = questionary.select(
        ...
        choices=[
            ...
            Choice("22) Withdraw ReactorFusion", withdraw_reactorfusion),
            Choice("23) Create NFT collection on Omnisea", create_omnisea),
            Choice("24) Mint and bridge NFT L2Telegraph", bridge_nft),
            Choice("25) Mint Tavaera ID + NFT", mint_tavaera),
            Choice("26) Mint NFT", mint_nft),
            Choice("27) Mint ZKS Domain", mint_zks_domain),
            Choice("28) Mint Era Domain", mint_era_domain),
            Choice("29) Send message L2Telegraph", send_message),
            Choice("30) Dmail sending mail", send_mail),
            Choice("31) MultiSwap", swap_multiswap),
            Choice("32) Use custom routes", custom_routes),
            Choice("33) MultiApprove", multi_approve),
            Choice("34) Deploy contract and mint token", deploy_contract_zksync),
