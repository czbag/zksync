...
def get_module():
    result = questionary.select(
        ...
        choices=[
            ...
            Choice("22) Create NFT collection on Omnisea", create_omnisea),
            Choice("23) Mint and bridge NFT L2Telegraph", bridge_nft),
            Choice("24) Mint Tavaera ID + NFT", mint_tavaera),
            Choice("25) Mint NFT", mint_nft),
            Choice("26) Mint ZKS Domain", mint_zks_domain),
            Choice("27) Mint Era Domain", mint_era_domain),
            Choice("28) Send message L2Telegraph", send_message),
            Choice("29) Dmail sending mail", send_mail),
            Choice("30) MultiSwap", swap_multiswap),
            Choice("31) Use custom routes", custom_routes),
            Choice("32) MultiApprove", multi_approve),
            Choice("33) Deploy contract and mint token", deploy_contract_zksync),
