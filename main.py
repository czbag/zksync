...
def get_module():
    result = questionary.select(
        ...
        choices=[
            ...
            Choice("36) Mint MailZero NFT", mint_mailzero_nft),
            Choice("37) Mint NFT", mint_nft),
            Choice("38) Mint ZKS Domain", mint_zks_domain),
            Choice("39) Mint Era Domain", mint_era_domain),
            Choice("40) Send message L2Telegraph", send_message),
            Choice("41) Dmail sending mail", send_mail),
            Choice("42) MultiSwap", swap_multiswap),
            Choice("43) Use custom routes", custom_routes),
            Choice("44) MultiApprove", multi_approve),
            Choice("45) Deploy contract and mint token", deploy_contract_zksync),
            Choice("46) Check transaction count", "tx_checker"),
            Choice("47) Exit", "exit"),
        ],
        qmark="⚙️ ",
        pointer="✅ "
