...
def get_module():
    result = questionary.select(
        ...
        choices=[
            ...
            Choice("21) Deposit Basilisk", deposit_basilisk),
            Choice("22) Withdraw Basilisk", withdraw_basilisk),
            Choice("23) Deposit ReactorFusion", deposit_reactorfusion),
            Choice("24) Withdraw ReactorFusion", withdraw_reactorfusion),
            Choice("25) Create NFT collection on Omnisea", create_omnisea),
            Choice("26) Mint and bridge NFT L2Telegraph", bridge_nft),
            Choice("27) Mint Tavaera ID + NFT", mint_tavaera),
            Choice("28) Mint NFT", mint_nft),
            Choice("29) Mint ZKS Domain", mint_zks_domain),
            Choice("30) Mint Era Domain", mint_era_domain),
            Choice("31) Send message L2Telegraph", send_message),
            Choice("32) Dmail sending mail", send_mail),
            Choice("33) MultiSwap", swap_multiswap),
            Choice("34) Use custom routes", custom_routes),
            Choice("35) MultiApprove", multi_approve),
            Choice("36) Deploy contract and mint token", deploy_contract_zksync),
