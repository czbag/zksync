...
def get_module():
    result = questionary.select(
        ...
        choices=[
            ...
            Choice("19) Mint ZKS Domain", mint_zks_domain),
            Choice("20) Mint Era Domain", mint_era_domain),
            Choice("21) Deploy contract and mint token", deploy_contract_zksync),
            Choice("22) Dmail sending mail", send_mail),
            Choice("23) MultiSwap", swap_multiswap),
            Choice("24) Stargate bridge MAV", stargate_bridge),
            Choice("25) Use custom routes", custom_routes),
            Choice("26) MultiApprove", multi_approve),
